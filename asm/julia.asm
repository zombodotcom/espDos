...
;
; Loaded as JULIA.COM. Renders 30 Julia-set frames (78x24, 16-color
; ANSI) to the 86-DOS console using INT 21h AH=09h PRTBUF (one row
; per call). Between frames we emit ESC[H to home the cursor so the
; next frame overwrites the previous in place.
;
; The parameter c walks a circle of radius 0.7885 around the origin
; in 30 steps (12 degrees per step). That circle crosses many of the
; visually distinct Julia "atlases": dust, dendrites, spirals,
; seahorses. After 30 frames (~one full revolution) we INT 20h back
; to SHELL.COM.
;
...
frame_loop:
    ...
    ; Cursor home so each frame overwrites the previous in place.
    push    cs
    pop     ds
    mov     ah, 0x09
    mov     dx, esc_home
...
pal_ok:
    ...
    ; Frame done. Loop until we've rendered NUM_FRAMES.
    inc     word [frame_var]
    mov     ax, [frame_var]
    cmp     ax, NUM_FRAMES
    jl      frame_loop

    ; All frames done — reset color and exit cleanly to SHELL.
    push    cs
    pop     ds
    mov     ah, 0x09
    mov     dx, esc_reset
...
; c-orbit table: 30 (c_real, c_imag) Q4.12 pairs walking a radius
; 0.7885 circle in 12-degree steps. Generated by:
;   py -c "import math; r=0.7885; ..."
NUM_FRAMES equ 30
c_table:
