/*
 * Vendored ST7735 driver — originally waveshare/esp_lcd_st7735 v1.0.1
 * (MIT license) from
 *   https://github.com/waveshareteam/Waveshare-ESP32-components
 *   path: display/lcd/esp_lcd_st7735
 *   commit: c3fb26108996924bd208a80719e88531dc6a1ab7
 *
 * Vendored rather than pulled via idf_component.yml because the
 * upstream uses the IDF 5.x field name `rgb_endian` and enum
 * `LCD_RGB_ENDIAN_*`, both renamed in IDF 6.0 to `rgb_ele_order`
 * and `LCD_RGB_ELEMENT_ORDER_*`. The patch is local and trivial;
