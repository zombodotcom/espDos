...
#ifdef ESP_PLATFORM
/* PATCH (ESP-IDF): newlib on Espressif has no <sys/timeb.h>. The only
 * use of struct timeb is in the GET_RTC opcode handler, which DOS 1.0
 * never triggers (it has its own date prompt and doesn't use the PC
 * BIOS RTC). We stub the type and ftime() so the file still compiles. */
struct timeb { unsigned int millitm; };
static inline int ftime(struct timeb *t) { t->millitm = 0; return 0; }
#else
#include <sys/timeb.h>
#endif
...
#ifdef ESP_PLATFORM
/* PATCH (ESP-IDF): the ESP-IDF newlib lacks <memory.h> (deprecated
 * legacy header). string.h above gives us memcpy/memset which is all
 * 8086tiny actually needs from it. */
