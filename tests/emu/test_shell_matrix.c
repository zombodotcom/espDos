...
static const char *script   = "1-1-80\rMATRIX\r";
...
int main(void) {
    ...
    /* MATRIX prints "ESC[32m" (set foreground green) per glyph. With
     * 80 cols * 100 frames, we should see thousands of these. Just
     * confirm at least one ANSI green appeared — proves MATRIX is
     * actually rendering rather than crashing or no-op'ing. */
    fails += T_EXPECT(strstr(out_buf, "\x1b[1;32m") != NULL);
