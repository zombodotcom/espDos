...
/* Date, then SNAKE, then nothing. SNAKE polls AH=06 DL=FF for keys
 * and only quits on 'q'; with no further input it should crawl
 * straight right until it hits the right wall. The original symptom
 * was `je .quit` getting NASM-emitted as `0F 84 ..` (286+) which
 * 8086tiny decoded as POP CS — corrupting CS and ending the game at
 * frame 1. With cpu 8086 forcing 8086-only encodings, SNAKE should
 * draw many frames of the snake glyph before any death. */
static const char *script   = "1-1-80\rSNAKE\r";
...
int main(void) {
    ...
    /* SNAKE draws its body cells using `snake_glyph` (defined in
     * snake.asm as a green '#'). If the game ended at frame 1 the
     * stream has only the initial 5 glyphs from the body init; if
     * it actually ran for many frames, we'd see lots more.
     * Count occurrences of "ESC[" — each cursor_to call emits one,
     * so >= 100 implies SNAKE ran for many frames. */
    int csi_count = 0;
    for (size_t i = 0; i + 1 < out_len; i++) {
        if ((unsigned char)out_buf[i] == 0x1B && out_buf[i+1] == '[') csi_count++;
    }
    printf("counted %d ESC[ sequences\n", csi_count);
    fails += T_EXPECT(csi_count >= 100);
